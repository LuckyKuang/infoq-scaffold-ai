...
```bash
node .codex/skills/infoq-frontend-verify/scripts/start_admin_dev_stack.mjs --client react --backend-port 8081 --profile local
```
...
状态文件为 `doc/tmp/infoq-frontend-verify/react/state.json`，记录 backend/frontend 的 pid、port 与日志。验证完成、失败或中断后必须执行 stop 命令；stop 只关闭本 skill 启动或状态文件记录为 owned 的进程，不强杀外部已有服务。
...
```bash
pnpm --dir .codex/skills/infoq-browser-automate/scripts run playwright-cli admin-route-probe --backend-url "http://127.0.0.1:9090" --frontend-origin "http://127.0.0.1/react" --route "/index"
```
