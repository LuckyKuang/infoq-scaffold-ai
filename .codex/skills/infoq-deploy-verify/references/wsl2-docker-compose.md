...
- Docker Compose 部署不要求宿主机安装 JDK 或 Maven；后端 Maven 打包在 `maven:3.9.12-eclipse-temurin-17` Docker builder 中执行，运行镜像保持 `bellsoft/liberica-openjdk-rocky:17.0.16-cds`。
...
| Debian 13 找不到 `openjdk-17-jdk` | 发行版 apt 源不提供 JDK 17，但 Compose 部署不使用宿主机 JDK | 不要把它作为 Docker Compose 部署 blocker；只确认 Docker 能拉取 Maven builder 和 BellSoft runtime 镜像 |
