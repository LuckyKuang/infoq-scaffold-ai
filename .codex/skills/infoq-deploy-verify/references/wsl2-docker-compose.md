...
- `SECURITY_TOKEN_SECRET` 必须提供，至少 32 字符，验证环境可用本地随机值。
...
```powershell
wsl.exe -d Debian -- bash -lc 'cd /mnt/c/DevTools/code/github/infoq-scaffold-ai && mkdir -p doc/tmp/deploy-wsl2 /tmp/infoq-deploy /infoq/server/temp && export INFOQ_DEPLOY_ROOT=/infoq SECURITY_TOKEN_SECRET=local-wsl2-deploy-secret-20260706-abcdef123456 && bash script/bin/infoq.sh deploy > doc/tmp/deploy-wsl2/backend-deploy.log 2>&1; status=$?; echo $status > doc/tmp/deploy-wsl2/backend-deploy.exit; tail -n 80 doc/tmp/deploy-wsl2/backend-deploy.log; exit $status'
```
...
readiness 需要返回 `200 OK`，body 中 `database.status` 和 `redis.status` 都为 `UP`。

## 前端与 nginx 部署

```powershell
wsl.exe -d Debian -- bash -lc 'cd /mnt/c/DevTools/code/github/infoq-scaffold-ai && mkdir -p doc/tmp/deploy-wsl2 /tmp/infoq-deploy /infoq/server/temp && export INFOQ_DEPLOY_ROOT=/infoq SECURITY_TOKEN_SECRET=local-wsl2-deploy-secret-20260706-abcdef123456 && bash script/bin/deploy-frontend.sh deploy > doc/tmp/deploy-wsl2/frontend-deploy.log 2>&1; status=$?; echo $status > doc/tmp/deploy-wsl2/frontend-deploy.exit; tail -n 100 doc/tmp/deploy-wsl2/frontend-deploy.log; exit $status'
```

成功信号：日志包含 `[frontend] 部署完成`，并提示 `/vue/`、`/react/`、`/react-pro/`、`Vue=9091 React=9092 ReactPro=9093`。
...
| React Pro build 中 Corepack `Cannot find matching keyid` | Node 22 镜像 Corepack keyring 与 pnpm 签名漂移 | Dockerfile 显式 `npm install -g pnpm@<packageManager版本>` |
