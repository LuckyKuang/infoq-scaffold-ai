...
5. 环境预检：确认 Docker daemon、Compose、JDK/Maven、Node/pnpm 镜像拉取能力、端口 `80/443/3306/6379/9000/9001/9090/9091/9092/9093`、`SECURITY_TOKEN_SECRET` 和 `INFOQ_DEPLOY_ROOT`。
...
7. 后端部署：执行 `script/bin/infoq.sh deploy`，先验证 MySQL/Redis/MinIO/backend，再进入前端。
8. 前端部署：执行 `script/bin/deploy-frontend.sh deploy`，顺序构建 Vue、React、React Pro 镜像并启动 nginx。
9. smoke 验证：检查容器状态、backend readiness/liveness、nginx `/prod-api` 反代、`/vue/`、`/react/`、`/react-pro/`、三个直连端口、MySQL 初始化数据和 Redis。
...
```bash
cd /path/to/infoq-scaffold-ai
INFOQ_DEPLOY_ROOT=/infoq SECURITY_TOKEN_SECRET=local-wsl2-deploy-secret-20260706-abcdef123456 bash script/bin/deploy-frontend.sh stop
INFOQ_DEPLOY_ROOT=/infoq SECURITY_TOKEN_SECRET=local-wsl2-deploy-secret-20260706-abcdef123456 bash script/bin/infoq.sh stop
```
...
- React Pro Docker build 若在 Node 22 镜像内出现 Corepack pnpm key mismatch，优先让 Dockerfile 显式安装与 `packageManager` 一致的 pnpm 版本。
