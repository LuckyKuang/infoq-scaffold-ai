...
这份 README 只描述 `infoq-scaffold-frontend-weapp-vue` 当前仓库实现，不把它抽象成通用 uni-app 模板。

## 当前实现概览

`infoq-scaffold-frontend-weapp-vue` 是当前仓库的 uni-app + Vue 3 小程序工作区，同时覆盖 H5 和微信小程序目标。应用入口是 `src/main.ts`，页面注册入口是 `src/pages.json`；请求统一走 `src/api/request.ts`，会话状态集中在 `src/store/session.ts`，页面级鉴权辅助集中在 `src/composables/use-auth-guard.ts`。

当前代码能直接确认的几个关键事实：

- `src/pages.json` 显式注册 `home/login/profile/notices/admin/system-*/monitor-*` 页面。
...
- `src/composables/use-auth-guard.ts` 提供 `ensureAuthenticated()` 与 `ensurePermission()`，由页面在加载时主动调用。
- `package.json` 内置 `build-open:weapp*`、`test:e2e:weapp:*`、`verify:build` 和 `verify:local`，用于 DevTools 与 e2e 验证闭环。
...
| `src/composables`、`src/components`、`src/utils` | 页面级鉴权、公共组件、导航与运行时辅助 | [`src/README.md`](./src/README.md) |

## 建议阅读顺序

1. [`doc/architecture.md`](./doc/architecture.md)
   先看页面注册、请求层、会话层和页面级鉴权分界。
...
- `pnpm run typecheck` 是 Vue 小程序工作区的编译门；`verify:build` 会固定执行 `typecheck -> test -> build:h5 -> build:weapp:dev -> build:weapp`。
...
- 当前 `pnpm install` 还会应用本地补丁 `patches/jsencrypt@3.5.4.patch` 和 `patches/miniprogram-automator@0.12.1.patch`，因此排查构建或 DevTools 自动化差异时不能忽略 `patches/`。
