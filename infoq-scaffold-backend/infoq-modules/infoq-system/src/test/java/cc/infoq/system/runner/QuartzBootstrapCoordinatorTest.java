...
class QuartzBootstrapCoordinatorTest {
    ...
    void reconcileShouldSkipWhenMarkerExists() {
        ...
        when(markerStore.hasMarker("infoq:quartz:bootstrap:applied:prod:2.1.6-20260602120000")).thenReturn(true);
        ...
        verify(markerStore).hasMarker("infoq:quartz:bootstrap:applied:prod:2.1.6-20260602120000");
        verify(markerStore, never()).markApplied("infoq:quartz:bootstrap:applied:prod:2.1.6-20260602120000");
    }
    ...
    void reconcileShouldRunAndWriteMarkerWhenGuarded() {
        ...
        when(environment.acceptsProfiles(Profiles.of("prod"))).thenReturn(true);
        when(lockTemplate.lock("infoq:quartz:bootstrap-reconcile:prod", -1L, 3000L)).thenReturn(lockInfo);
        when(markerStore.hasMarker("infoq:quartz:bootstrap:applied:prod:2.1.6-20260602120000")).thenReturn(false);
        ...
        verify(lockTemplate).releaseLock(lockInfo);
        verify(markerStore).hasMarker("infoq:quartz:bootstrap:applied:prod:2.1.6-20260602120000");
        verify(markerStore).markApplied("infoq:quartz:bootstrap:applied:prod:2.1.6-20260602120000");
    }
    ...
    void reconcileShouldUseNewDeployIdAsDistinctMarker() {
        QuartzManagedProperties properties = properties(true, true, true);
        properties.getBootstrap().setDeployId("2.1.6-20260602123000");
        QuartzBootstrapCoordinator coordinator = new QuartzBootstrapCoordinator(properties, lockTemplate, environment, markerStore);
        Runnable runnable = mock(Runnable.class);
        LockInfo lockInfo = new LockInfo("k", "v", -1L, 3000L, 1, new Object(), mock(LockExecutor.class));
        when(environment.acceptsProfiles(Profiles.of("prod"))).thenReturn(true);
        when(lockTemplate.lock("infoq:quartz:bootstrap-reconcile:prod", -1L, 3000L)).thenReturn(lockInfo);
        when(markerStore.hasMarker("infoq:quartz:bootstrap:applied:prod:2.1.6-20260602123000")).thenReturn(false);

        boolean executed = coordinator.reconcile(runnable);

        assertTrue(executed);
        verify(runnable).run();
        verify(markerStore).hasMarker("infoq:quartz:bootstrap:applied:prod:2.1.6-20260602123000");
        verify(markerStore).markApplied("infoq:quartz:bootstrap:applied:prod:2.1.6-20260602123000");
        verify(lockTemplate).releaseLock(lockInfo);
    }
    ...
    void markerKeyAndLockKeyShouldIncludeProfile() {
        ...
        assertEquals("infoq:quartz:bootstrap:applied:prod:2.1.6-20260602120000", coordinator.markerKey());
    ...
    private static QuartzManagedProperties properties(boolean reconcileEnabled, boolean productionGuardEnabled, boolean markerEnabled) {
        ...
        properties.getBootstrap().setDeployId("2.1.6-20260602120000");
