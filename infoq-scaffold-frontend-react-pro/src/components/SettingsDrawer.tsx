...
import {Button, ColorPicker, Divider, Drawer, message, Switch, theme as antdTheme, Typography,} from 'antd';
...
export default function SettingsDrawer({ open, onClose }: SettingsDrawerProps) {
  ...
  const toggleFixedHeader = useSettingsStore(
    (state) => state.toggleFixedHeader,
  );
  const toggleSidebarLogo = useSettingsStore(
    (state) => state.toggleSidebarLogo,
  );
  const toggleDynamicTitle = useSettingsStore(
    (state) => state.toggleDynamicTitle,
  );
  ...
  return (
    ...
    >
      <div
        style={{ display: 'flex', minHeight: '100%', flexDirection: 'column' }}
      >
        <div>
