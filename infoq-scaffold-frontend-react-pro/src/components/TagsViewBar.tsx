...
export default function TagsViewBar({ activePath }: TagsViewBarProps) {
  ...
  const selectedView = useMemo(
    () => visitedViews.find((item) => item.path === selectedPath),
    [selectedPath, visitedViews],
  );
  ...
  const navigateToLatest = () => {
    ...
    history.push(latest?.path || '/index');
  };
  ...
  const handleCloseLeft = (view: TagView) => {
    ...
    const hasCurrent = useTagsViewStore
      .getState()
      .visitedViews.some((item) => item.path === activePath);
    if (!hasCurrent) {
      history.push(view.path);
    }
  };
  ...
  const openMenu = useCallback(
    (view: TagView, event: ReactMouseEvent<HTMLSpanElement>) => {
      ...
      visitedViews.map((view) => ({
        ...
        label: (
          <span
            ...
            onContextMenu={(event) => openMenu(view, event)}
          >
            {tagsIcon && view.icon ? (
              <SvgIcon
                iconClass={view.icon}
