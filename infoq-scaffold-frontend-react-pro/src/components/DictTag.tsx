...
export default function DictTag({
  ...
  const matched = options.filter((item) =>
    values.some((val) => String(item.value) === val),
  );
  const unmatched = values.filter(
    (val) => !options.some((item) => String(item.value) === val),
  );
      ...
      {matched.map((item) => {
        ...
        return (
          <Tag
            ...
            color={toTagColor(item.elTagType)}
            className={item.elTagClass}
          >
            {item.label}
          </Tag>
        );
