  ...
  const uploadProps: UploadProps = {
    ...
    beforeUpload(file) {
      const fileExt = file.name.includes('.')
        ...
        : '';
      const isImg =
        fileType.includes(fileExt) || file.type.startsWith('image/');
      if (!isImg) {
        modal.msgError(
          `文件格式不正确，请上传 ${fileType.join('/')} 图片格式文件`,
        );
    ...
    onChange(info) {
      ...
      if (info.file.status === 'done') {
        const resp = info.file.response as {
          code?: number;
          data?: { ossId?: string | number; fileName?: string; url?: string };
          msg?: string;
        };
      ...
      {preview && (
        <Image
          ...
          preview={{
            visible: true,
            src: preview,
            onVisibleChange: (v) => !v && setPreview(''),
          }}
        />
      )}
      {showTip && (
        <div style={{ color: 'rgba(0, 0, 0, 0.45)', marginTop: 6 }}>
          请上传大小不超过 <b style={{ color: '#ff4d4f' }}>{fileSize}MB</b>
