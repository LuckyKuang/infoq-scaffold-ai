...
type SearchParamsCarrier = Record<string, unknown> & {
...
export const selectDictLabel = (
  ...
  const matched = toDictEntries(datas).find(
...
export const parseStrEmpty = <T extends PrimitiveValue>(
  str: T,
  ...
  for (const node of nodes) {
    ...
    if (!parentObj || parentObj === node) {
      ...
      const exists = childList.some(
        (child) => child && child[config.id] === node[config.id],
      );
      if (!exists) {
        childList.push(node);
      }
