...
export default function RegisterPage() {
  ...
  const handleSendCode = async () => {
    try {
      const fields = inviteRegisterEnabled
        ? ['inviteCode', 'email']
        : ['email'];
      if (captchaEnabled) {
        fields.push('code');
  ...
  const onFinish = async (values: RegisterForm) => {
    ...
    try {
      ...
      message.success(
        t('register.registerSuccess', { username: values.username }),
      );
      navigate('/login');
    } catch {
      if (captchaEnabled) {
      ...
      <Form form={form} onFinish={onFinish}>
        ...
        {inviteRegisterEnabled && (
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: t('register.rule.inviteCode.required'),
              },
              {
          ...
          >
            <Input
              ...
              prefix={
                <SvgIcon iconClass="password" size={14} style={authIconStyle} />
              }
        ...
        >
          <Input
            ...
            prefix={
              <SvgIcon iconClass="email" size={14} style={authIconStyle} />
            }
          />
        </Form.Item>
        ...
        >
          <Form.Item
            ...
            rules={[
              {
                required: true,
                message: t('register.rule.emailCode.required'),
              },
            ]}
          >
            <Input
          ...
          <Button
            ...
            disabled={
              countdown > 0 ||
              (inviteRegisterEnabled && (inviteChecking || !inviteCodeValid))
            }
        ...
        {captchaEnabled && (
          ...
          >
            <Form.Item
              name="code"
              rules={[
                { required: true, message: t('register.rule.code.required') },
              ]}
        ...
        >
          <Input
            ...
            prefix={
              <SvgIcon iconClass="user" size={14} style={authIconStyle} />
            }
          />
        </Form.Item>
        ...
        <Form.Item
          ...
          rules={[
            { required: true, message: t('register.rule.password.required') },
            {
              min: 8,
              max: 30,
              message: t('register.rule.password.length', { min: 8, max: 30 }),
            },
            {
            ...
            ({ getFieldValue }) => ({
              validator(_, value) {
                ...
                return Promise.reject(
                  new Error(t('register.rule.confirmPassword.equalToPassword')),
                );
              },
            }),
          ]}
