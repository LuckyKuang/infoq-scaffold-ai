...
import type {
  DataSourceMonitorSummary,
  DataSourceMonitorVO,
  DataSourcePoolVO,
} from '@/api/monitor/dataSource/types';
...
const normalizeMonitor = (
  value?: Partial<DataSourceMonitorVO> | null,
): DataSourceMonitorVO => {
...
export default function DataSourcePage() {
  const [monitor, setMonitor] =
    useState<DataSourceMonitorVO>(createDefaultMonitor);
  ...
  useEffect(() => {
    const loadMonitor = async () => {
      ...
      try {
        ...
        setErrorMessage(
          error instanceof Error ? error.message : '加载连接池监控数据失败',
        );
      } finally {
        modal.closeLoading();
      }
        ...
        render: (value: number | null) => {
          ...
          return (
            <Space orientation="vertical" size={4} style={{ width: '100%' }}>
              <Typography.Text
                style={{ color: usage >= 80 ? '#cf1322' : undefined }}
              >
                {formatPercent(usage)}
              </Typography.Text>
              <Progress
                percent={usage}
                size="small"
                status={usage >= 90 ? 'exception' : undefined}
              />
            </Space>
          );
        },
      ...
      <Row gutter={[12, 12]}>
        <Col xs={12} md={6}>
          <Card hoverable>
            <Statistic
              title="数据源数"
              value={monitor.summary.dataSourceCount}
        ...
        <Col xs={12} md={6}>
          <Card hoverable>
            <Statistic
              title="活跃连接"
              value={monitor.summary.activeConnections}
              prefix={<LinkOutlined />}
            />
          </Card>
        </Col>
        <Col xs={12} md={6}>
          <Card hoverable>
            <Statistic
              title="空闲连接"
              value={monitor.summary.idleConnections}
              prefix={<DatabaseOutlined />}
            />
          </Card>
        </Col>
        <Col xs={12} md={6}>
          <Card hoverable>
            <Statistic
              title="等待线程"
              value={monitor.summary.threadsAwaitingConnection}
