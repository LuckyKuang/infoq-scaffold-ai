...
export default function CachePage() {
  ...
  useEffect(() => {
    ...
    if (
      !commandChartElement ||
      !memoryChartElement ||
      (!cache.commandStats.length && !cache.info.used_memory_human)
    ) {
      return undefined;
    }
    ...
    const hasValidChartSize = (size: { width: number; height: number }) =>
      size.width > 0 && size.height > 0;

    const initializeCharts = () => {
      const chartSizes = readChartSizes();
      if (
        !hasValidChartSize(chartSizes.command) ||
        !hasValidChartSize(chartSizes.memory)
      ) {
        return;
      }
    ...
    const resizeObserver = new ResizeObserver(() => {
      ...
      const commandSizeChanged =
        chartSizes.command.width !== lastCommandChartSize.width ||
        chartSizes.command.height !== lastCommandChartSize.height;
      const memorySizeChanged =
        chartSizes.memory.width !== lastMemoryChartSize.width ||
        chartSizes.memory.height !== lastMemoryChartSize.height;
  ...
  const cpuUsage =
    cache.info.used_cpu_user_children !== undefined
      ? Number.parseFloat(cache.info.used_cpu_user_children).toFixed(2)
      : '-';
  const redisMode =
    cache.info.redis_mode === undefined
      ? '-'
      : cache.info.redis_mode === 'standalone'
        ? '单机'
        : '集群';
  const aofEnabled =
    cache.info.aof_enabled === undefined
      ? '-'
      : cache.info.aof_enabled === '0'
        ? '否'
        : '是';
  const networkIO =
    cache.info.instantaneous_input_kbps !== undefined &&
    cache.info.instantaneous_output_kbps !== undefined
      ...
      <Row gutter={[10, 10]}>
        ...
        <Col xs={24} md={12} className="card-box">
          ...
          >
            <div
              ref={commandChartRef}
              data-testid="cache-command-chart"
              style={{ height: 420 }}
            />
          </Card>
        </Col>
        ...
        <Col xs={24} md={12} className="card-box">
          ...
          >
            <div
              ref={memoryChartRef}
              data-testid="cache-memory-chart"
              style={{ height: 420 }}
            />
          </Card>
        </Col>
