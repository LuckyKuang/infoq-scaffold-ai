...
const toSelectTree = (nodes: DeptTreeVO[]): TreeSelectOption[] =>
  nodes.map((node) => ({
    value: node.id,
    title: node.label,
...
export default function PostPage() {
  ...
  const columns: ColumnsType<PostVO> = [
    ...
    { title: '排序', dataIndex: 'postSort', width: 80, align: 'center' },
    {
      ...
      align: 'center',
      render: (value: string) => (
        <DictTag options={dict.sys_normal_disable || []} value={value} />
      ),
    },
  ...
  const handleDelete = async (
    postId?: string | number | Array<string | number>,
  ) => {
    ...
    const confirmed = await modal.confirm(
      `是否确认删除岗位编号为 "${Array.isArray(target) ? target.join(',') : target}" 的数据项？`,
    );
    if (!confirmed) {
      return;
    }
