...
const toTreeSelectData = (nodes: DeptVO[]): TreeSelectOption[] =>
  nodes.map((node) => ({
    value: node.deptId,
    title: node.deptName,
...
const collectDeptIds = (nodes: DeptVO[]): Array<string | number> =>
  nodes.flatMap((node) =>
    node.children?.length
      ? [node.deptId, ...collectDeptIds(node.children)]
      : [],
  );
...
export default function DeptPage() {
  ...
  const handleDelete = async (deptId?: string | number, deptName?: string) => {
    ...
    const confirmed = await modal.confirm(
      `是否确认删除名称为 "${deptName || deptId}" 的数据项？`,
    );
    if (!confirmed) {
      return;
    }
                ...
                >
                  <Input
                    ...
                    onChange={(event) =>
                      setQuery((prev) => ({
                        ...prev,
                        deptCategory: event.target.value,
                      }))
                    }
                    onPressEnter={() => loadList({ ...query, pageNum: 1 })}
                  />
                ...
                <Form.Item style={{ marginBottom: 0 }}>
                  <Space>
                    <Button
                      type="primary"
                      icon={<SearchOutlined />}
                      onClick={() => loadList({ ...query, pageNum: 1 })}
                    >
                      搜索
                    </Button>
                    <Button
        ...
        <Table<DeptVO>
          ...
          expandable={{
            ...
            onExpandedRowsChange: (keys) =>
              setExpandedRowKeys(keys as Array<string | number>),
          }}
        />
      </Card>
