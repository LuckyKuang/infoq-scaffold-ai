...
const getAllItems = (panel: ConfigPanel) =>
  panel.groups.flatMap((group) => group.items);
const maskValue = (value?: string) =>
  value ? '•'.repeat(Math.min(Math.max(value.length, 6), 12)) : '未设置';
const hasDefaultValue = (item: ConfigPanelItem) =>
...
export default function ConfigPage() {
  ...
  const loadPanel = useCallback(async () => {
    ...
    try {
      ...
      modal.msgError(
        error instanceof Error ? error.message : '配置面板加载失败',
      );
    } finally {
      setLoading(false);
    }
    ...
    return panel.groups
      .map((group) => ({
        ...
        items: group.items.filter((item) => {
          const matchedGroup =
            activeGroupKey === 'all' || item.groupKey === activeGroupKey;
      ...
      .filter(
        (group) => group.items.length > 0 || activeGroupKey === group.groupKey,
      );
  ...
  const pagedGroups = useMemo(() => {
    ...
    const pagedItems = filteredGroups
      .flatMap((group) =>
        group.items.map((item) => ({ groupKey: group.groupKey, item })),
      )
      .slice(start, end);
  ...
  useEffect(() => {
    const maxPage = Math.max(
      1,
      Math.ceil(filteredItemTotal / listPagination.limit),
    );
    ...
    setPanel((prev) => ({
      groups: prev.groups.map((group) => ({
        ...
        items: group.items.map((item) =>
          item.configKey === configKey ? { ...item, configValue } : item,
        ),
      })),
    }));
  };
  ...
  const saveItemValue = async (
    item: ConfigPanelItem,
    nextValue: string | boolean,
  ) => {
    const previousValue = item.configValue;
    const normalizedValue =
      typeof nextValue === 'boolean' ? toBooleanString(nextValue) : nextValue;
  ...
  const openEditDrawer = (item: ConfigPanelItem) => {
    form.setFieldsValue({
      ...
      uiPropsJson:
        item.uiProps && Object.keys(item.uiProps).length > 0
          ? JSON.stringify(item.uiProps)
          : null,
  ...
  const renderValueControl = (item: ConfigPanelItem) => {
    ...
    if (item.valueType === 'text' || item.valueType === 'password') {
      ...
      return (
        <Space wrap className="config-edit-row">
          <Text code>
            {item.valueType === 'password'
              ? maskValue(item.configValue)
              : item.configValue || '未设置'}
          </Text>
          <Tooltip
            title={
              item.editable ? '编辑当前值' : item.editableReason || '不可编辑'
            }
          >
            <Button
              icon={<EditOutlined />}
    ...
    return (
      <Tag color="default">{item.valueType || 'text'} 类型暂不支持面板编辑</Tag>
    );
  };
  ...
  const renderConfigCard = (item: ConfigPanelItem) => (
    <Card
      key={item.configKey}
      className="config-center-card config-setting-row"
      size="small"
    >
      <div
        className={`config-card-main ${editingKeys[item.configKey] ? 'config-card-main-editing' : ''}`}
      >
        <div className="config-card-copy">
          <Space size={8} wrap className="config-card-heading">
            <Text strong>{item.configName}</Text>
            <Tag color={item.configType === 'Y' ? 'blue' : 'default'}>
  ...
  const orderColumns: ColumnsType<ConfigReorderForm> = [
    {
      ...
      dataIndex: 'configId',
      render: (value) =>
        getAllItems(panel).find((item) => item.configId === value)
          ?.configName || value,
    },
    {
      title: '分组',
        ...
        <InputNumber
          ...
          onChange={(nextOrder) =>
            setOrderRows((prev) =>
              prev.map((item) =>
                item.configId === row.configId
                  ? { ...item, displayOrder: Number(nextOrder ?? 0) }
                  : item,
              ),
            )
          }
    ...
    <div className="config-center-page">
        ...
        <div className="config-hero-content">
          <div>
            ...
            <Text type="secondary" className="config-subtitle">
              按类型直接修改配置，恢复默认由后端读取 default_value
              执行，旧配置缺元数据时归入高级配置。
            </Text>
          </div>
      ...
      <div className="config-center-layout">
        <Card className="config-group-nav" size="small">
          <Button
            block
            type={activeGroupKey === 'all' ? 'primary' : 'text'}
              ...
              <div className="config-group-list">
                {filteredItemTotal === 0 ? (
                  ...
                  pagedGroups.map((group: ConfigPanelGroup) => (
                    <section
                      key={group.groupKey}
                      className="config-group-section"
                    >
                      ...
                      <div className="config-card-list">
                        {group.items.map(renderConfigCard)}
                      </div>
                    </section>
                  ))
                )}
              ...
              children: (
                <Form form={form} layout="vertical" initialValues={initialForm}>
                  ...
                  <Form.Item
                    label="默认值"
                    name="defaultValue"
                    tooltip="留空提交时表示无默认值；空字符串默认值后续由高级模式处理。"
                  >
                    {watchedValueType === 'switch' ? (
                      <Select
                        allowClear
                  ...
                  {watchedValueType === 'select' && (
                    <Form.Item
                      ...
                      name="optionsJson"
                      rules={[
                        { required: true, message: '下拉类型必须配置选项' },
                      ]}
                    >
                      <Input.TextArea
                        rows={4}
                        placeholder='[{"label":"浅色主题","value":"theme-light"}]'
                      />
                    </Form.Item>
                  )}
                  ...
                  <Space>
                    ...
                    <Button onClick={() => form.setFieldsValue(initialForm)}>
                      清空表单
                    </Button>
                  </Space>
                </Form>
