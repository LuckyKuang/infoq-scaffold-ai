...
export default function ClientPage() {
  ...
  const columns: ColumnsType<ClientVO> = [
    ...
    },
    {
      ...
      align: 'center',
      render: (value: string[]) => (
        <DictTag options={dict.sys_grant_type || []} value={value || []} />
      ),
    },
    {
      title: '设备类型',
  ...
  const handleDelete = async (
    ...
    const confirmed = await modal.confirm(
      `是否确认删除客户端编号为 "${Array.isArray(target) ? target.join(',') : target}" 的数据项？`,
    );
    if (!confirmed) {
      return;
    }
      ...
      >
        <Form form={form} layout="vertical" initialValues={initialForm}>
          ...
          >
            <Select
              mode="multiple"
              options={(dict.sys_grant_type || []).map((item) => ({
                label: item.label,
                value: item.value,
              }))}
            />
          ...
          >
            <Select
              options={(dict.sys_device_type || []).map((item) => ({
                label: item.label,
                value: item.value,
              }))}
            />
