...
const hasRoutePath = (routes: AppRoute[], targetPath: string): boolean => {
  return routes.some(
    (route) =>
      ...
      (route.children ? hasRoutePath(route.children, targetPath) : false),
  );
};
...
const withHomeMeta = (routes: AppRoute[]): AppRoute[] => {
  return routes.map((route) => {
    ...
    const icon =
      next.meta?.icon && next.meta.icon !== '#'
        ? next.meta.icon
        : HOME_ROUTE.meta?.icon;
...
type PermissionState = {
  ...
  getRouteByComponent: (
    component?: string,
  ) => RouteComponentMapItem | undefined;
};
...
export const usePermissionStore = create<PermissionState>((set, get) => ({
  ...
  setRoutes: (newRoutes) =>
    set((state) => ({
      addRoutes: newRoutes,
      routes: [...state.defaultRoutes, ...newRoutes],
    })),
  ...
  generateRoutes: async () => {
    ...
    const rewriteRoutes = withAbsoluteRoutePaths(
      filterAsyncRouter(rdata, true),
    );
    const defaultRoutes = withAbsoluteRoutePaths(
  ...
  getRouteByComponent: (component) => {
    ...
    return (
      all.find((item) => item.component === component && item.meta?.title) ||
      all.find((item) => item.component === component)
    );
  },
}));
