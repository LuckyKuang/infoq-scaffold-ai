...
const normalizeTagPath = (path: string) =>
...
const dedupeVisited = (views: TagView[], next: TagView) => {
  ...
  const index = views.findIndex(
    (item) => normalizeTagPath(item.path) === incomingPath,
  );
  if (index < 0) {
  ...
  const merged = {
    ...
    path: incomingPath,
    affix:
      views[index].affix || normalizedNext.affix || isHomeTag(normalizedNext),
  };
...
export const useTagsViewStore = create<TagsViewState>((set) => ({
  ...
  delView: (path) => {
    set((state) => {
      const target = state.visitedViews.find((item) => item.path === path);
      const visitedViews = state.visitedViews.filter(
        (item) => item.path !== path || item.affix,
      );
      const cachedViews = target
        ? state.cachedViews.filter((item) => item !== target.name)
        : state.cachedViews;
      return {
        visitedViews,
        cachedViews,
  ...
  delLeftViews: (path) => {
    set((state) => {
      ...
      const visitedViews = state.visitedViews.filter(
        (item, idx) => idx >= index || item.affix,
      );
      const cachedViews = state.cachedViews.filter(
        (item) => !removedNames.has(item),
      );
      return {
        visitedViews,
        cachedViews,
