...
describe('access', () => {
  ...
  it('should expose permission helpers from backend codes', () => {
    const result = access({
      roles: ['common'],
      permissions: ['system:user:add'],
    });

    expect(result.hasPermi('system:user:add')).toBe(true);
    expect(result.hasPermiOr(['system:user:remove', 'system:user:add'])).toBe(
      true,
    );
