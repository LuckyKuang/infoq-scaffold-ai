...
describe('api/type-contract', () => {
  it('checks login API contracts', () => {
    ...
    expectTypeOf(login).returns.toMatchTypeOf<
  ...
  it('checks public self-service auth API contracts', () => {
    ...
    expectTypeOf(forgotPassword)
      .parameter(0)
      .toMatchTypeOf<ForgotPasswordForm>();
  });

  it('checks verify code and user info contracts', () => {
    expectTypeOf(getCodeImg).returns.toMatchTypeOf<
      Promise<ApiResponse<VerifyCodeResult>>
    >();
    expectTypeOf(getInfo).returns.toMatchTypeOf<
      Promise<ApiResponse<UserInfo>>
    >();
  });

  it('checks router API contracts', () => {
    expectTypeOf(getRouters).returns.toMatchTypeOf<
      Promise<ApiResponse<AppRoute[]>>
    >();
  });
});
