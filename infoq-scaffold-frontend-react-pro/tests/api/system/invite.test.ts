...
describe('api/system/invite', () => {
  ...
  it('lists invite codes with query params', async () => {
    const { listInvite } = await import('@/api/system/invite');
    listInvite({
      pageNum: 1,
      pageSize: 10,
      inviteCode: 'INVITE',
  ...
  it('generates, cancels and deletes invite codes', async () => {
    const { cancelInvite, delInvite, generateInvite } = await import(
      '@/api/system/invite'
    );
    generateInvite({
      generateCount: 10,
