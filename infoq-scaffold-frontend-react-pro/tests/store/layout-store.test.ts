...
describe('store/layout', () => {
  ...
  it('normalizes shorthand hex and falls back for invalid values', () => {
    ...
    expect(normalizeThemeColor('invalid-theme')).toBe(
      defaultLayoutSettings.theme,
    );
  });
  ...
  it('supports tags add/close/closeOthers/closeLeft/closeRight/closeAll', () => {
    ...
    expect(
      useTagsViewStore.getState().visitedViews.map((item) => item.path),
    ).toEqual(['/index', '/system/user']);
    ...
    expect(
      useTagsViewStore.getState().visitedViews.map((item) => item.path),
    ).toEqual(['/index', '/system/role', '/monitor/cache']);

    useTagsViewStore.getState().delOthersViews('/system/role');
    expect(
      useTagsViewStore.getState().visitedViews.map((item) => item.path),
    ).toEqual(['/index', '/system/role']);
  ...
  it('keeps home tag at the far left even when added after business tags', () => {
    ...
    expect(
      useTagsViewStore.getState().visitedViews.map((item) => item.path),
    ).toEqual(['/index', '/system/dept/index', '/system/menu/index']);
  });
});
