...
describe('store/permission', () => {
  ...
  it('normalizes sidebar children to absolute paths and prepends home when missing', async () => {
    ...
    const { sidebarRouters, getRouteByPath, getRouteByComponent } =
    ...
    expect(getRouteByPath('/monitor/cache')?.component).toBe(
      'monitor/cache/index',
    );
    expect(getRouteByPath('/index')?.component).toBe('index');
    expect(getRouteByComponent('system/user/index')?.meta?.title).toBe(
      '用户管理',
    );
  });
