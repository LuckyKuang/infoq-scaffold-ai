...
describe('store/user', () => {
  ...
  it('login success should update token and initialize realtime channels', async () => {
    userStoreMocks.login.mockResolvedValue({
  ...
  it('oauth ticket login should exchange ticket and update token through same session path', async () => {
    userStoreMocks.exchangeOAuthTicket.mockResolvedValue({
      data: { access_token: 'oauth-token' },
    } as ApiResponse<LoginResult>);

    await useUserStore.getState().loginByOAuthTicket('ticket-1');

    expect(userStoreMocks.exchangeOAuthTicket).toHaveBeenCalledWith({
