...
const { default: DataSourcePage } = await import(
  '@/pages/monitor/dataSource/index'
);
...
describe('pages/dataSource', () => {
  ...
  it('loads datasource monitor data and only renders safe pool summary fields', async () => {
    ...
    expect(dataSourceMocks.modalLoading).toHaveBeenCalledWith(
      '正在加载连接池监控数据，请稍候！',
    );
    ...
    expect(
      screen.queryByText('jdbc:mysql://localhost:3306/infoq'),
  ...
  it('shows an error when datasource monitor response omits items', async () => {
    ...
    expect(
      await screen.findByText('连接池监控响应 items 必须是数组'),
    ).toBeInTheDocument();
  });
});
