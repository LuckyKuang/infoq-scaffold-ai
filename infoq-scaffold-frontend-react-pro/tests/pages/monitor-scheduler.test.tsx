...
beforeEach(() => {
  ...
  vi.mocked(jobApi.delJob).mockResolvedValue(
    asResolvedValue<Awaited<ReturnType<typeof jobApi.delJob>>>(undefined),
  );
  vi.mocked(jobApi.runJob).mockResolvedValue(
    asResolvedValue<Awaited<ReturnType<typeof jobApi.runJob>>>(undefined),
  );
  vi.mocked(jobApi.changeJobStatus).mockResolvedValue(
    asResolvedValue<Awaited<ReturnType<typeof jobApi.changeJobStatus>>>(
      undefined,
    ),
  );
  ...
  vi.mocked(jobLogApi.delJobLog).mockResolvedValue(
    asResolvedValue<Awaited<ReturnType<typeof jobLogApi.delJobLog>>>(undefined),
  );
  vi.mocked(jobLogApi.cleanJobLog).mockResolvedValue(
    asResolvedValue<Awaited<ReturnType<typeof jobLogApi.cleanJobLog>>>(
      undefined,
    ),
  );
});
...
describe('pages/monitor scheduler', () => {
  it('renders job page and supports run/delete/export', async () => {
    ...
    expect(
      await screen.findByPlaceholderText('请输入任务名称'),
    ).toBeInTheDocument();
    ...
    const runButton = runButtons.find((button) =>
      button.querySelector('.anticon-play-circle'),
    );
    if (!runButton) {
      throw new Error('Expected run job button to be rendered');
    }
    ...
    expect(requestMocks.download).toHaveBeenCalledWith(
      '/monitor/job/export',
      expect.any(Object),
