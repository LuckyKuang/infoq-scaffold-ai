...
describe('pages/cache', () => {
  ...
  beforeEach(() => {
    ...
    vi.spyOn(
      ...
      'clientHeight',
      'get',
    ).mockImplementation(() => chartHeight);
  ...
  it('loads cache metrics and initializes pie and gauge charts after container sizes are ready', async () => {
    ...
    expect(
      await screen.findByTestId('cache-command-chart'),
    ).toBeInTheDocument();
    ...
    expect(cacheMocks.modalLoading).toHaveBeenCalledWith(
      '正在加载缓存监控数据，请稍候！',
    );
      ...
      expect.objectContaining({
        series: [
          expect.objectContaining({
            type: 'gauge',
            data: [
              expect.objectContaining({ value: 123.45, name: '内存消耗' }),
            ],
          }),
        ],
      }),
